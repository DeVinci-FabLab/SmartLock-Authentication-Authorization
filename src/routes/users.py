...
router = APIRouter(
    tags=["User Management (Keycloak)"],
    dependencies=[Depends(require_admin)]
)

# --- Schémas Pydantic spécifiques à cette route ---
...
async def get_all_users(
    search: Optional[str] = Query(None, description="Recherche par nom, email ou username"),
    first: int = 0,
    max_results: int = 100
):
...
async def add_new_user(user: UserCreateRequest):
