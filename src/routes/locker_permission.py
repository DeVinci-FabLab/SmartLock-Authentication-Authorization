...
from src.schemas.locker_permission import LockerPermissionCreate, LockerPermissionUpdate, LockerPermissionResponse
...
def create_permission(locker_id: int, permission: LockerPermissionCreate, db: Session = Depends(get_db)):
    if locker_id != permission.locker_id:
        raise HTTPException(status_code=400, detail="Locker ID dans l'URL et dans le body ne correspondent pas.")
    try:
...
def update_permission(permission_id: int, permission: LockerPermissionUpdate, db: Session = Depends(get_db)):
